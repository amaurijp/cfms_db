...
class WV(object):
    ...
    def get_LSA_doc_topic_matrix(self):
        ...
        print('\n( Function: get_LSA_doc_topic_matrix )')
        ...
        if os.path.exists(self.diretorio + f'/Outputs/TFIDF/H5/doc_topic_full_rNgram_{self.replace_Ngrams}.h5'):
            ...
            last_batch_processed = int(load_index_from_TXT(f'counter_batch_DOC_TOPIC_rNgram_{self.replace_Ngrams}', '/Outputs/LOG', diretorio = self.diretorio))
            ...
            save_index_to_TXT(0, f'counter_batch_DOC_TOPIC_rNgram_{self.replace_Ngrams}', '/Outputs/LOG', diretorio = self.diretorio)
        ...
        for batch in range(next_batch, last_batch_to_be_processed + 1):
            ...
            first_pdf_filename , last_pdf_filename = self.proc_documents[first_file_index] , self.proc_documents[last_file_index]

            #determinando o index do primeiro documento (sent) do batch
            initial_sent_index = sent_indexes.loc[first_pdf_filename, 'initial_sent']            
            #determinando o index do último documento (sent) do batch
            last_sent_index = sent_indexes.loc[last_pdf_filename, 'last_sent']

            #carregando o arquivo TFIDF.npz do batch
            m = sparse.load_npz(self.diretorio + f'/Outputs/TFIDF/npz/sparse_csr_rNgram_{self.replace_Ngrams}_{c_batch_number}.npz')
            print('Carregando a matrix TFIDF do batch. Shape: ', m.shape)
            print('Carregando a matrix term_topic. Shape: ', svd_wv.values.shape)
                        
            #carregando a matriz DOC_TOPIC
            h5 = h5py.File(self.diretorio + f'/Outputs/TFIDF/H5/doc_topic_full_rNgram_{self.replace_Ngrams}.h5', 'a')
            document_topic_m = h5['data']
            print('Carregando a matrix doc_topic. Shape: ', document_topic_m.shape)
                        
            print('Filenames - first: ', first_pdf_filename, ' ; last: ', last_pdf_filename)
            print('Sent slice: ', initial_sent_index, ' a ',  last_sent_index)
            
            #Dot product das matrizes: DOC_TERM . TERM_TOPIC
            document_topic_m[ initial_sent_index : last_sent_index + 1] = np.dot( m.todense(), svd_wv.values )              
            
            #salvando o número total de sentença (documents)
            save_index_to_TXT(batch, f'counter_batch_DOC_TOPIC_rNgram_{self.replace_Ngrams}', '/Outputs/LOG', diretorio = self.diretorio)            
...
def split_TFIDF_to_SPARSE(self, batch_size = 50000):
    ...
    print('\n( Function: split_TFIDF_to_SPARSE )')
